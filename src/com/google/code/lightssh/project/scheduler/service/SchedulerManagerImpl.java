...
import com.google.code.lightssh.common.support.quartz.MyCronTriggerFactoryBean;
...
public class SchedulerManagerImpl implements SchedulerManager{
	...
	@Override
	public void restore(String triggerName, String group) {
		JobInterval jobInterval = new JobInterval();
		jobInterval.setTriggerName(triggerName);
		JobInterval interval = jobIntervalManager.get(jobInterval);
		if(interval==null){
			throw new ApplicationException("表中不存在此任务："+triggerName);
		}
		jobIntervalManager.remove(jobInterval = new JobInterval());
		
		CronTrigger  bean = (CronTrigger )SpringContextHelper.getBean(triggerName);
		String object = bean.getCronExpression();
		Trigger trigger = null;
		try{
			trigger = scheduler.getTrigger( TriggerKey.triggerKey(triggerName, group) );
		}catch(Exception e ){
			throw new ApplicationException(e);
		}
		freshTrigger(trigger,true,object);
	}

	@Override
	public void saveCronExp(String name, String group, String cron) {
		Trigger trigger = null;
		try{
			trigger = scheduler.getTrigger( TriggerKey.triggerKey(name, group) );
		}catch(Exception e ){
		...
		jobInterval.setCronExpression(cron);
		jobInterval.setEnabled(enabled);
		if( insert )
			this.jobIntervalManager.create(jobInterval);
		else
		...
		freshTrigger(trigger,enabled,cron);
	}

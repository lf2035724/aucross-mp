...
public class SchedulerAction extends BaseAction{
	...
	public String restore(){
		try{
			schedulerManager.restore(name, group);
		}catch( Exception e ){
			this.saveErrorMessage("任务操作失败：" + e.getMessage() );
		}
	...
	public String mysaveTrigger(){
		...
		CronValidate validate = new CronValidate(second, minute, hour, dayOfMonth, month, dayOfWeek);
		String cron = second+" "+minute +" "+hour+" "+ dayOfMonth+" "+ month+" "+ dayOfWeek;
		try{
			schedulerManager.saveCronExp(name,group,cron);
			this.saveSuccessMessage("更新时钟成功！");
		}catch( Exception e ){
			this.addActionError("更新时钟异常："+e.getMessage());
			return INPUT;

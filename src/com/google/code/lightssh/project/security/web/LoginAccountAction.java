...
import com.ylink.ylpay.common.project.mp.constant.LicensedType;
...
public class LoginAccountAction extends GenericAction<LoginAccount> {
	...
	private final String[] AUTH_TYPE_VALUES = { "hasAuth", "noneAuth", "canAuth", "genAuth", "secondAuth" };
...
//	}
	 ...
	 */
	public String view() {
		if ( account == null || ( account.getIdentity() == null && StringUtil.clean( account.getLoginName() ) == null ) )
			return INPUT;
	...
	public String save() {
		...
		if ( account.getPeriod() != null ) {
			...
			if ( account.getPeriod().getStart() == null && account.getPeriod().getEnd() == null )
		...
		try {
			...
			this.saveSuccessMessage( "登录帐户[" + account.getLoginName() + "]" + message + "成功，等待审核！" );
		} catch ( Exception e ) {
	...
	public String delete() {
		...
		try {
			...
			saveSuccessMessage( "停用操作已受理，审核通过后生效！" );
		} catch ( Exception e ) {
			saveErrorMessage( "停用发生异常：" + e.getMessage() );
			return INPUT;
		}
	...
	public String resetpassword() {
		if ( account == null || account.getLoginName() == null || passwords == null || passwords.isEmpty() ) {
		...
		log.info( "操作员[{}]成功重置帐号[{}]登录密码！", getLoginUser(), account.getLoginName() );
	...
	public String recovery() {
		...
		// clean Session
		request.getSession().setAttribute( SessionKey.LOGIN_ACCOUNT, null );
		return SUCCESS;
	}
	...
	public String emailretrieve() {
		...
		String checkMsg = CryptographyUtil.hashSha1Hex( account.getEmail() + account.getPassword() );
		...
		// 用于重置密码的"安全值"
		safeMessage = CryptographyUtil.hashSha1Hex( request.getSession().getId() + account.getLoginName() + account.getPassword() );
	...
	public String retrieve() {
		if ( safeMessage == null || account == null || account.getLoginName() == null )
			return ERROR;
		...
		// 用于重置密码的"安全值"
		String checkValue = CryptographyUtil.hashSha1Hex( request.getSession().getId() + account.getLoginName() + account.getPassword() );
	...
	public String validatePassword() {
		...
		if ( getLoginAccount() != null && getLoginAccount().getPassword() != null && getLoginAccount().getPassword().equals( password ) ) {
			if ( la.hasPermission( per.getToken() ) ) {
				this.passed = true;
				try {
					AuthorizedTicket ticket = authorizedTicketManager.authTicket( targetUrl, la.getLoginName(), per.getToken(),
							AuthorizedTicket.Scope.ONCE, request.getSession().getId(), getLoginUser() );
		...
		if ( !StringUtils.isEmpty( targetUrl ) ) {
			...
			if ( permission != null ) {
				...
				if(authResource != null ){
					//是否已存在授权
					AuthorizedTicket ticket = authorizedTicketManager.get( token, targetUrl, request.getSession().getId() );
					if ( ticket != null && ticket.isEffective() ) {
						authType = AUTH_TYPE_VALUES[ 3 ];
						this.ticket = ticket.getId();
					}else if(LicensedType.PWD_AUTH.getValue().equals(authResource.getLicensedType())){
						authType = AUTH_TYPE_VALUES[ 0 ];
					}else if(LicensedType.SECOND_AUTH.getValue().equals(authResource.getLicensedType())){
						authType = AUTH_TYPE_VALUES[ 4 ];
					}
				}
			}
		}
	...
	public String authResource() {
		...
		if ( username.equals(getLoginAccount().getLoginName())) {
			this.safeMessage = "登录用户与授权用户不能相同！";
			return SUCCESS;
		}
		...
		if ( account.hasPermission( per.getToken() ) ) {
			this.passed = true;
			try {
				AuthorizedTicket ticket = authorizedTicketManager.authTicket( targetUrl, username, per.getToken(),
						AuthorizedTicket.Scope.ONCE, request.getSession().getId(), getLoginUser() );
				this.ticket = ticket.getId();
			} catch ( Exception e ) {
				this.safeMessage = "授权异常：" + e.getMessage();
	...
	public String checkPassword() {
		String json = "false";
		String password = request.getParameter( "password" );
		if ( super.getLoginAccount().getPassword().equals( password ) ) {
			json = "true";
			log.info( "密码校验通过..." );
		}
	...
	public String checkLoginName() {
		...
		String propertyValue = request.getParameter( "loginName" );
		return this.check( propertyName, propertyValue );
	}
	...
	public String checkMobile() {
		...
		String propertyValue = request.getParameter( "mobile" );
		return this.check( propertyName, propertyValue );
	}
	...
	public String checkEmail() {
		...
		String propertyValue = request.getParameter( "email" );
		return this.check( propertyName, propertyValue );
	}
	 ...
	 * @return
	 */
	private String check( final String propertyName, final String propertyValue ) {
		String json = "false";
		...
		LoginAccount entity = this.getManager().get( id, propertyName, propertyValue );
		if ( null == entity ) {
			json = "true";
		}
		StrutsUtils.renderJson( json );

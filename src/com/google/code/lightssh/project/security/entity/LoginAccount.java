...
public class LoginAccount extends BaseModel implements Cloneable, Loggable{
	...
	@JoinTable( name = "T_REF_LOGINACCOUNT_ROLE", joinColumns = @JoinColumn( name = "LOGINACCOUNT_ID" ), inverseJoinColumns = @JoinColumn( name = "ROLE_ID" ) )
	...
	public boolean hasRole( String roleName ) {
		if ( roleName == null || this.getRoles() == null || this.getRoles().isEmpty() )
			return false;
	...
	public boolean hasPermission( String per ) {
		if ( per == null || this.getRoles() == null || this.getRoles().isEmpty() )
			return false;

		Collection<String> allPers = new ArrayList<String>();
		Collection<String> rolePers = null;
		for ( Role role : this.roles ) {
			rolePers = role.getPermissionsAsString();
			if(null != rolePers)
			allPers.addAll(rolePers);
		}
	...
	public String logMessage() {
		return "登录帐号 [邮箱=" + email + ",状态=" + this.status + ", 登录名=" + loginName + ",手机=" + mobile + ", 部门编号=" + partyId + ",有效期=" + period
				+ ",拥有角色=" + roles + "]";
	}

	@Override
	public String toString() {
		return "LoginAccount [email=" + email + ", enabled=" + this.isEnabled() + ", loginName=" + loginName + ", mobile=" + mobile
				+ ", partyId=" + partyId + ", period=" + period + ", roles=" + roles + "]";
	}

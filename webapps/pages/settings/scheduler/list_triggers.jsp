...
<html>
	<head>
		...
		<script type="text/javascript">
			function toggle(name,group,pause){
				...
				if( confirm('确认'+(pause?'暂停':'启动')+'定时任务?') )
					$.lightssh.checkAuth(
							{'contextPath': path
							,'checkPassword': true
							,'targetUrl':path+'/settings/scheduler/toggle.do'}
							,function(succes,ticket,msg){ 
								if( !succes )
									return;
								if( ticket != null || ticket != '' ){
									url = url + '&auth.ticket=' + ticket;
								}
								location.href=url;
							}
						);
			...
			function restore(name,group){
				var url='<s:url value="/settings/scheduler/restore.do?name="/>'+name+'&group='+group;
				if(confirm('确认恢复时钟【'+name+'】'))
					location.href=url;
			}
		</script>
	</head>
			...
			<mys:column title="操作" width="40px" cssClass="action">
				...
				<div class="popup-menu-layer">
					<ul class="dropdown-menu">
						<shiro:hasPermission name="SETTING_SCHEDULER_TOGGLE">
						...
						<li class="section"/>
						<shiro:hasPermission name="SETTING_SCHEDULER_EDIT">
						<li class="edit"><a href="<s:url value="edit.do?name=%{name}&group=%{group}"/>">更改时钟</a></li>
						</shiro:hasPermission>
						<shiro:hasPermission name="SETTING_SCHEDULER_MYEDIT">
						<li class="edit"><a href="<s:url value="myedit.do?name=%{name}&group=%{group}"/>">修改时钟</a></li>
						</shiro:hasPermission>
						<shiro:hasPermission name="SETTING_SCHEDULER_RESTORE">
						<li class="edit"><a href="#" onclick="restore('<s:property value="%{name}"/>','<s:property value="%{group}"/>');">恢复时钟</a></li>
						</shiro:hasPermission>
					</ul>
				</div>
			</mys:column>

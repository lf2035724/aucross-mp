...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$("#eauditForm").validate({
					rules:{
						"workOrderAudit.description":{required:true,maxlength:200}
					}
	...
	<body>
		...
		<%@ include file="/pages/mp/workorder/audit/compare.jsp" %>
		...
		<table class="profile">
			...
			<tbody>
				<tr>
					...
					<td><s:property value="workOrderChange.operator.loginName"/></td>
				</tr>
				<tr>
					<th><label>操作类型</label></th>
					<td><s:property value="workOrderChange.type"/></td>
				</tr>
				<tr>
					<th><label>操作时间</label></th>
					<td><s:property value="workOrderChange.createdTime"/></td>
				</tr>
				<tr>
					<th><label>操作备注</label></th>
					<td><s:property value="workOrderChange.description"/></td>
				</tr>
			</tbody>
		</table>
		
		<s:form id="eauditForm" action="audit.do" namespace="/workorder" method="post">
			<table class="profile">
				<caption>审核信息</caption>
				<tbody>
					<tr>
						<th><label class="required" for="desc">审核备注</label></th>
						<td><s:textarea id="desc" name="workOrderAudit.description" cols="60" rows="5"/></td>
					</tr>
				</tbody>
			</table>
			
			<p class="submit">
				<s:hidden name="workOrderChange.id"/>
				<s:hidden name="workOrderChange.workOrder.id"/>
